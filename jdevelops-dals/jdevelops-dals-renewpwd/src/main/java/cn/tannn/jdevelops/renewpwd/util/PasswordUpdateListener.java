 ...
 * <P> 2. 在 PwdRefreshUtil 中添加静态监听器字段和注册方法，密码更新成功时回调监听器。
 * <P> 3. 让 DatabasePwdEnvironmentPostProcessor 实现监听器接口，并在初始化时注册自己为监听器。
 * <P> 4. 当 PwdRefreshUtil.updateUserPassword 成功后，自动调用监听器的 onPasswordUpdated 方法，DatabasePwdEnvironmentPostProcessor 在回调中用新密码重新初始化 RenewPasswordService
 */
