...
import static cn.tannn.jdevelops.spi.StrUtils.isBlank;
import static cn.tannn.jdevelops.spi.StrUtils.isNotBlank;
    ...
    private void loadResources(final Map<String, Class<?>> classes, final URL url) throws IOException {
        try (InputStream inputStream = url.openStream()) {
            ...
            properties.forEach((k, v) -> {
                ...
                if (isNotBlank(name) && isNotBlank(classPath)) {
                    try {
                        // 加载扩展的实现类并缓存
