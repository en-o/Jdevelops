...
//        claims.setNotBeforeMinutesInThePast(1); //令牌尚未生效的时间 (2 minutes ago)
    ...
    public static String getSubjectExpires(String token) throws LoginException {
        try {
            ...
            throw new LoginException(TokenCode.TOKEN_ERROR, e);
        } catch (NullPointerException nullPointerException) {
            throw new LoginException(TokenCode.UNAUTHENTICATED_PLATFORM, nullPointerException);
        }
    }
    ...
    public static <T, S> T getTokenByBean(String token, Class<T> t, Class<S> ts) {
        ...
        Type type = new ParameterizedTypeImpl(t, ts);
    ...
    public static JwtClaims parseJwt(String token) {
        try {
            ...
            // 非法
            if (jwtClaims == null) {
                throw new LoginException(TokenCode.UNAUTHENTICATED_PLATFORM, e);
            }
            return jwtClaims;
        }
    }
