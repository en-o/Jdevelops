package com.detabes.redis.pub.config;

import com.detabes.redis.pub.entity.ReidsCacheBean;
import com.detabes.redis.pub.receiver.RedisReceiver;
...
public class RedisCacheConfig {
    ...
    RedisMessageListenerContainer container(RedisConnectionFactory connectionFactory,
        ...
        if(patternTopic==null||patternTopic.size()<=0){
            container.addMessageListener(listenerAdapter, new PatternTopic("test"));
        }else{
            patternTopic.forEach(str -> {
                container.addMessageListener(listenerAdapter, new PatternTopic(str));
            });
        }
